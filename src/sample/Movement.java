...
public abstract class Movement {
    ...
    static void moveHeroTo (Hero hero, double x, double y, GameAsServer gameAsServer) throws InstantiationException, IllegalAccessException {
        if (x >= 0 && x <= gameAsServer.W - hero.getBoundsInLocal().getWidth() && y >= 0 && y <= gameAsServer.H - hero.getBoundsInLocal().getHeight() && backgroundCheck(x,y,hero, gameAsServer)) {
            hero.relocate(x, y);
        } else if (x >= 0 && x <= gameAsServer.W -hero.getBoundsInLocal().getWidth() && backgroundCheck(x,y,hero)) {
            hero.relocate(x, hero.getLayoutY());
            hero.pos_y -= hero.dy;
        }
        else if (y>=0 && y<= gameAsServer.H -hero.getBoundsInLocal().getHeight() && backgroundCheck(x,y,hero)) {
            hero.relocate(hero.getLayoutX(), y);
            hero.pos_x -= hero.dx;
        } else {
            hero.pos_x -= hero.dx;
        ...
        hero.hpBar.setCoordinates(hero.getLayoutX()-4,hero.getLayoutY()-20);
        if(Hero.isWarrior(gameAsServer.heroes.get(0))) {
            swordMove(hero);
        }
        if(hero.barrierCheck)
        {
            ...
            {
                gameAsServer.pane.getChildren().remove(hero.barrier);
                hero.barrierCheck=false;
                hero.barrierTime=0;
            }
            counter++;
    ...
    static void moveVillain (GameAsServer gameAsServer) throws InstantiationException, IllegalAccessException {
        double d;
        Iterator<Villain> it=gameAsServer.villains.iterator();
        while(it.hasNext()) {
            Villain currentVillain = it.next();
            d = currentVillain.getSpeed();
            if (gameAsServer.heroes.get(0).getBoundsInParent().intersects(currentVillain.getBoundsInParent())) {
                if(!gameAsServer.heroes.get(0).barrierCheck) {
                    gameAsServer.heroes.get(0).hp--;
                    gameAsServer.hp_texts.get(gameAsServer.heroes.get(0)).setText("HP: " + gameAsServer.heroes.get(0).hp);
                    gameAsServer.heroes.get(0).changeHpBar();
                }
                else
                {
                    gameAsServer.heroes.get(0).barrierCheck=false;
                    gameAsServer.heroes.get(0).barrierTime=0;
                    gameAsServer.pane.getChildren().remove(gameAsServer.heroes.get(0).barrier);
                }
                if (Villain.isShooting(currentVillain)) {
                    gameAsServer.shootingVillains.remove(currentVillain);
                }
                it.remove();
                gameAsServer.pane.getChildren().remove(currentVillain);
                gameAsServer.pane.getChildren().remove(currentVillain.hpBar);
                continue;
            }
            double x = currentVillain.getLayoutX();
            double y = currentVillain.getLayoutY();
            double z = gameAsServer.heroes.get(0).getLayoutX();
            double v = gameAsServer.heroes.get(0).getLayoutY();
            double dd = Math.sqrt((x - z) * (x - z) + (y - v) * (y - v));
            boolean tt = villainBgCheck(currentVillain);
            boolean isBoss = currentVillain.isBoss();
            if (!isBoss) {
                if (tt) {
                    ...
                    double yy = curr.getLayoutY() + curr.getBoundsInLocal().getHeight() / 2;
                    double b = yy - y;
                    double s = Math.signum(-gameAsServer.heroes.get(0).getLayoutY() + currentVillain.getLayoutY());
                    if (b == 0) {
                        if (!(gameAsServer.heroes.get(0).getLayoutY() >= curr.getLayoutY() - gameAsServer.heroes.get(0).getBoundsInLocal().getHeight()  && gameAsServer.heroes.get(0).getLayoutY() <= curr.getLayoutY() + curr.getBoundsInLocal().getHeight() + 20)) {
                            if (xx < currentVillain.getLayoutX()) {
                                currentVillain.relocate(currentVillain.getLayoutX(), currentVillain.getLayoutY() - s * d);
                            } else {
                                currentVillain.relocate(currentVillain.getLayoutX(), currentVillain.getLayoutY() + s * d);
    ...
    static void throwWeapon(double d, GameAsServer gameAsServer) throws InstantiationException, IllegalAccessException {
        Iterator<Weapon> z=gameAsServer.weaponsHero.iterator();
        while(z.hasNext()){
            Weapon x=z.next();
            double dd=Math.sqrt(x.x*x.x+x.y*x.y);
            if (x.getLayoutX()<= Game.W && x.getLayoutX()>=0 && x.getLayoutY()<= Game.H && x.getLayoutY()>=0 && backgroundCheck(x.getLayoutX() + d*x.x/dd,x.getLayoutY() + d * x.y / dd,x)){
                ...
                z.remove();
                gameAsServer.pane.getChildren().remove(x);
            }
        }
    }
    ...
    public static void enemyWeapon(double d, GameAsServer gameAsServer) throws InstantiationException, IllegalAccessException {
        Iterator<Weapon> z=gameAsServer.weaponsVillain.iterator();
        while(z.hasNext()){
            Weapon x=z.next();
            double dd=Math.sqrt(x.x*x.x+x.y*x.y);
            if (x.getLayoutX()<= Game.W && x.getLayoutX()>=0 && x.getLayoutY()<= Game.H && x.getLayoutY()>=0 && backgroundCheck(x.getLayoutX() + d*x.x/dd,x.getLayoutY() + d * x.y / dd,x)){
    ...
    private static boolean backgroundCheck(double x, double y, ImageView object) {
        Iterator<Background> it;
        if (!Server.serverCreated) {
            it=Game.game.backgroundObjects.iterator();
        } else {
            it=GameAsServer.game.backgroundObjects.iterator();
        }
    ...
    private static boolean backgroundCheck(double x, double y, ImageView object, GameAsServer gameAsServer) {
        Iterator<Background> it=gameAsServer.backgroundObjects.iterator();
        ImageView test=new ImageView(object.getImage());
        test.relocate(x, y);
        while(it.hasNext())
        {
    ...
    private static boolean villainBgCheck(Villain object) {
        if (!Server.serverCreated) {
            ...
            for (Background current : GameAsServer.game.backgroundObjects) {
                double dd = Math.sqrt((object.getLayoutX() - current.getLayoutX() - current.getBoundsInLocal().getWidth() / 2) * (object.getLayoutX() - current.getLayoutX() - current.getBoundsInLocal().getWidth() / 2) + (object.getLayoutY() - current.getLayoutY() - current.getBoundsInLocal().getHeight() / 2) * (object.getLayoutY() - current.getLayoutY() - current.getBoundsInLocal().getHeight() / 2));
                if (dd <= 60) {
                    curr = current;
                    return false;
